...
bool CDatabase::LoadBots()
{
	...
	while ((row = mysql_fetch_row(result)))
	{
		...
		i++;
		bot.email = row[i] ? row[i] : "";
		i++;

		// location
		bot.id_planet = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.universe = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.galaxy = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.system = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.planet = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.onlinetime = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.vacation_mode = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.vacation_until = row[i] ? std::stoi(row[i]) : 0;
		i++;

		// research queue
		bot.b_tech_planet = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.b_tech = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.b_tech_id = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.b_tech_queue = row[i] ? row[i] : "";
		i++;

		// technologies
		bot.resource[106] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[108] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[109] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[110] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[111] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[113] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[114] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[115] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[117] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[118] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[120] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[121] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[122] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[123] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[124] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[131] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[132] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[133] = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.resource[199] = row[i] ? std::stoi(row[i]) : 0;
		i++;

		// commanders
		bot.rpg_geologist = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_admiral = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_engineer = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_technocrat = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_espion = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_constructor = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_scientist = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_commander = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_stocker = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_defender = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_destructor = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_general = row[i] ? std::stoi(row[i]) : 0;
		i++;
		bot.rpg_bunker = row[i] ? std::stoi(row[i]) : 0;
		i++;
	...
	while ((row = mysql_fetch_row(result)))
	{
		...
		switch (item_class)
		{
			case 0:
				...
				break;
			case 100:
				reslist_.tech.push_back(id);
				break;
			case 200:
				reslist_.fleet.push_back(id);
				break;
			case 400:
				reslist_.defense.push_back(id);
				break;
			case 500:
				reslist_.missile.push_back(id);
				break;
			case 600:
				reslist_.officers.push_back(id);
				break;
			case 700:
				reslist_.dmfunc.push_back(id);
				break;
		}
...
bool CDatabase::LoadConfig()
{
	...
	while ((row = mysql_fetch_row(result)))
	{
		...
		item.uni = std::stoi(row[i]);
		i++;
		item.game_speed = std::stoll(row[i]);
		i++;
		item.resource_multiplier = std::stoi(row[i]);
		i++;
		item.storage_multiplier = std::stoi(row[i]);
		i++;
		item.metal_basic_income = std::stoi(row[i]);
		i++;
		item.crystal_basic_income = std::stoi(row[i]);
		i++;
		item.deuterium_basic_income = std::stoi(row[i]);
		i++;
		item.max_galaxy = std::stoi(row[i]);
		i++;
		item.max_system = std::stoi(row[i]);
		i++;
		item.max_planet = std::stoi(row[i]);
		i++;
...
bool CDatabase::LoadSettlementData()
{
	...
	while ((row_planets = mysql_fetch_row(result_planets)))
	{
		size_t i = 0;
		item.Reset();
		item.galaxy = row_planets[i] ? std::stoi(row_planets[i]) : 0;
		i++;
		item.system = row_planets[i] ? std::stoi(row_planets[i]) : 0;
		i++;
		item.planet = row_planets[i] ? std::stoi(row_planets[i]) : 0;
		i++;
		item.universe = row_planets[i] ? std::stoi(row_planets[i]) : 0;
		i++;
	...
	while ((row_fleets = mysql_fetch_row(result_fleets)))
	{
		size_t i = 0;
		item.Reset();
		item.galaxy = row_fleets[i] ? std::stoi(row_fleets[i]) : 0;
		i++;
		item.system = row_fleets[i] ? std::stoi(row_fleets[i]) : 0;
		i++;
		item.planet = row_fleets[i] ? std::stoi(row_fleets[i]) : 0;
		i++;
		item.universe = row_fleets[i] ? std::stoi(row_fleets[i]) : 0;
		i++;
		settlement_data_.push_back(item);
		num++;
	}
...
bool CDatabase::AddBots(std::vector<create_info>& bots)
{
	...
	{
		query_update +=
			"WHEN " + std::to_string(firstUserID + i) + " THEN " + std::to_string(firstPlanetID + i) + " ";
	}
